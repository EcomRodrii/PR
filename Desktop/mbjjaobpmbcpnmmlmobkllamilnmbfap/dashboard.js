...
const els = {
  ...
  setupAnalysisBlock: document.getElementById('setup-analysis-block'),
  setupProductName: document.getElementById('setup-product-name'),
  setupUnitCost: document.getElementById('setup-unit-cost'),
  setupBrowserIdSummary: document.getElementById('setup-browser-id-summary'),
...
function buildAutoSearchUrl(productName) {
  const term = String(productName || '').trim();
  const encoded = encodeURIComponent(term);
  return `https://www.vinted.es/catalog?search_text=${encoded}&order=newest_first&page=1&status_ids[]=6&status_ids[]=1`;
}
...
function renderBrowserIdSummary(config) {
  const box = els.browserIdSummary;
  const browserId = formatBrowserId(config);
  const text = `ID de sesion: ${browserId}`;
  if (box) {
    box.textContent = text;
  }
  if (els.setupBrowserIdSummary) {
    els.setupBrowserIdSummary.textContent = text;
  }
}
...
function setSetupMode(enabled) {
  ...
  app.setupMode = next;
  document.body.classList.toggle('setup-mode', next);
  if (els.setupScreen) {
    els.setupScreen.hidden = !next;
  }
  if (!next) {
...
function setSetupMessage(text, tone = 'muted') {
  if (!els.setupMsg) return;
  els.setupMsg.textContent = String(text || '');
  els.setupMsg.dataset.tone = tone;
}

function setSetupLoading(visible, text = 'Espera a que configuremos todo.') {
  const isVisible = visible === true;
  if (els.setupLoadingOverlay) {
    els.setupLoadingOverlay.hidden = !isVisible;
  }
  if (els.setupLoadingText && text) {
    els.setupLoadingText.textContent = String(text);
  }
  document.body.classList.toggle('setup-loading', isVisible);
}

function setSetupAnalysisLocked(locked) {
  const isLocked = locked === true;
  if (els.setupAnalysisBlock) {
    els.setupAnalysisBlock.classList.toggle('is-locked', isLocked);
  }
  if (els.setupProductName) {
    els.setupProductName.disabled = isLocked;
  }
  if (els.setupUnitCost) {
    els.setupUnitCost.disabled = isLocked;
  }
  if (els.setupStartMonitor) {
    els.setupStartMonitor.disabled = isLocked;
  }
}


function resolveLatestActivityIso(config, state) {
  const metrics = state?.metrics || {};
  const candidates = [
    metrics.lastTrackRun,
    metrics.lastDetectRun,
    metrics.lastMonitorStartAt,
    metrics.firstMonitorStartAt,
    config?.vintedSession?.lastCheckedAt,
  ];
  let latestIso = null;
  let latestMs = 0;
  for (const raw of candidates) {
    const ms = getValidTimeMs(raw);
    if (!ms) continue;
    if (ms > latestMs) {
      latestMs = ms;
      latestIso = String(raw);
    }
  }
  return latestIso;
}
...
async function createAnalysisFromMenu() {
  ...
  try {
    ...
    const startResult = response?.result?.start || {};
    if (startResult?.ok === false) {
      throw new Error(startResult?.error || 'create_analysis_failed');
    }
    await loadState();
...
async function activateAnalysisFromMenu(analysisId) {
  ...
  try {
    ...
    const startResult = response?.result?.start || {};
    if (startResult?.ok === false) {
      throw new Error(startResult?.error || 'activate_analysis_failed');
    }
    await loadState();
    if (source === 'setup') {
      await waitMs(Math.max(0, SETUP_LOADING_MIN_MS - (Date.now() - startedAt)));
...
function syncSetupFieldsFromConfig(config) {
  if (!els.setupProductName) return;
  const cfgName = typeof config?.productName === 'string' ? config.productName : '';
  const cfgUnitCost = formatUnitCostForInput(config?.unitCost);

  if (document.activeElement !== els.setupProductName && els.setupProductName.value !== cfgName) {
    els.setupProductName.value = cfgName;
  }
  if (els.setupUnitCost && document.activeElement !== els.setupUnitCost && els.setupUnitCost.value !== cfgUnitCost) {
    els.setupUnitCost.value = cfgUnitCost;
  }
}

function renderVintedAccountSummary(config) {
  const box = els.vintedAccountSummary;
  if (!box) return;
  const s = config?.vintedSession || {};
  const parts = [];
  parts.push(`Cuenta Vinted: ${formatVintedSessionShort(s)}`);
  parts.push(`estado: ${s.status || 'unknown'}`);
  if (s.memberId) parts.push(`id: ${s.memberId}`);
  if (s.profileUrl) parts.push(`perfil: ${s.profileUrl}`);
  if (s.language || s.locale) {
    parts.push(`idioma: ${s.language || '-'}${s.locale ? ` (${s.locale})` : ''}`);
  }
  if (s.lastCheckedAt) {
    parts.push(`check: ${fmtDate(s.lastCheckedAt)}`);
  }
  if (s.reason) {
    parts.push(`motivo: ${s.reason}`);
  }
...
function renderKpis(items, metrics) {
  ...
  const conversion = totalDetected > 0 ? (sold / totalDetected) * 100 : 0;
  ...
  els.kpiConversion.textContent = `${conversion.toFixed(1)}%`;
...
function renderAll() {
  ...
  syncSetupFieldsFromConfig(app.config || {});
  renderBrowserIdSummary(app.config || {});
  renderAnalysisMaturity();
  renderDeterministicCountdown();
  updateExportButtonState();
  if (els.analysisMenuButton) {
  ...
  const browserId = formatBrowserId(app.config || {});
  ...
  if (!app.setupMode) {
    setSetupMode(!monitorEnabled);
    if (!monitorEnabled) {
      setSetupAnalysisLocked(false);
      setSetupMessage('Define el producto y pulsa iniciar para arrancar el analisis.', 'muted');
    }
  }
  const monitorStatus = app.config?.monitorEnabled
    ? `Monitor activo (${cfgName || 'sin nombre'}) | analisis: ${analysisLabel} | links: ${cfgUrls.length} | nav: ${browserId}`
    : hasAnalysisStarted(app.state)
      ? `Analisis en pausa | analisis: ${analysisLabel} | nav: ${browserId}`
      : `Monitor detenido | analisis: ${analysisLabel} | nav: ${browserId}`;
  updateStatusLine(app.state.metrics || {}, monitorStatus);
  app.filters = currentFilters();
  renderKpis(items, app.state.metrics || {});
  ...
  app.filteredItems = applyFilters(monitoringItems, app.filters);
...
async function startMonitorFlow({ productName, searchUrls, unitCost = null, button, source = 'dashboard' }) {
  ...
  try {
    ...
    if (!response?.success || !response?.result?.ok) {
      ...
      const startNotification = response?.result?.startNotification || {};
      let notifyText = '';
      if (startNotification.sent === true) {
        notifyText = ' | Discord: aviso de inicio enviado';
      } else if (startNotification.error) {
        notifyText = ` | Discord: fallo aviso inicio (${startNotification.error})`;
      }
      const msg = `Monitor iniciado: ${productName} (${searchUrls.length} links)${notifyText}`;
      updateStatusLine(app.state?.metrics || {}, msg);
      ok = true;
    }
    await loadState();
  ...
  try {
    ...
    if (action === 'rb:run-track') {
      if (result?.disabled) {
        ...
        const notifyErrors = Number(result?.notifyErrors || 0);
        const sellerChecked = Number(result?.sellerChecked || 0);
        const sellerErrors = Number(result?.sellerErrors || 0);
        updateStatusLine(
          app.state?.metrics || {},
          `Analizados: ${result.checked ?? 0}/${result.attempted ?? 0} | errores: ${result.errors ?? 0} | expirados: ${result.expired ?? 0} | avisos Discord: ${notified}${notifyErrors > 0 ? ` (fallos: ${notifyErrors})` : ''} | seller: ${sellerChecked}${sellerErrors > 0 ? ` (fallos: ${sellerErrors})` : ''}`
        );
      }
    }
    ...
    if (action === 'rb:reset-all') {
      ...
      if (action === 'rb:stop-monitoring') {
        const stopNotification = result?.stopNotification || {};
        let notifyText = '';
        if (stopNotification.sent === true) {
          notifyText = ' | Discord: aviso de pausa enviado';
        } else if (stopNotification.error) {
          notifyText = ` | Discord: fallo aviso pausa (${stopNotification.error})`;
        }
        updateStatusLine(app.state?.metrics || {}, `Analisis en pausa${notifyText}`);
      }
  } catch (err) {
...
function exportAnalysisReportCsv({ items, state, config, activeAnalysis, gateInfo = null }) {
  ...
  lines.push(csvRow(['resumen', 'informe', 'Blackstock Report CSV']));
  ...
  lines.push(csvRow(['resumen', 'cuenta', String(config?.accountEmail || '-')]));
  ...
  lines.push(csvRow(['kpi', 'conversion_pct', kpis.conversion.toFixed(2)]));
  ...
  a.download = `blackstock_informe_${slug}_${datePart}.csv`;
...
function bindEvents() {
  ...
  if (els.setupStartMonitor) {
    els.setupStartMonitor.addEventListener('click', async () => {
      ...
      const searchUrls = productName ? [buildAutoSearchUrl(productName)] : [];
      const unitCost = parseUnitCostInput(els.setupUnitCost?.value);

      if (els.productName) {
        els.productName.value = productName;
      }
      if (els.searchUrls) {
        els.searchUrls.value = searchUrls.join('\n');
      }
  ...
  if (els.setupProductName) {
    const syncSetupSearchUrl = () => {
      ...
      const generated = buildAutoSearchUrl(name);
    ...
    els.saveUrl.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!response?.success) {
          ...
          updateStatusLine(
            app.state?.metrics || {},
            `Links guardados: ${searchUrls.length} | Discord: activo (fijo)`
          );
        }
        await loadState();
      } finally {
...
async function initDashboard() {
  ...
  bindEvents();
  await loadState();
  const isFirstUse = !app.config?.monitorEnabled && !app.config?.productName &&
    (!Array.isArray(app.state?.items) || app.state.items.length === 0);
  if (isFirstUse) {
    const productName = 'rayban';
    const searchUrls = [buildAutoSearchUrl(productName)];
    try {
      await send('rb:start-monitoring', { productName, searchUrls });
      await loadState();
    } catch (_) {
      // no-op
    }
